...
#        self.load_state_dict('./vgg16_bn-6c64b313.pth')

class DepthNet(nn.Module):
    def __init__(self, channel=64):
        super(DepthNet, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, channel, 3, padding=1, stride=2, bias=False), nn.BatchNorm2d(channel), nn.PReLU()
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(channel, channel, 3, padding=1, stride=1, bias=False), nn.BatchNorm2d(channel), nn.PReLU()
        )
    ...
    def forward(self, d):
        d2 = self.conv1(d)
        d3 = F.max_pool2d(d2, kernel_size=3, stride=2, padding=1)
        d3 = self.conv2(d3)
        d4 = F.max_pool2d(d3, kernel_size=3, stride=2, padding=1)
        d4 = self.conv3(d4)
        d5 = F.max_pool2d(d4, kernel_size=3, stride=2, padding=1)
        d5 = self.conv4(d5)
...
class SGNet_vgg16(nn.Module):
    def __init__(self, cfg):#, cfg
        ...
        self.decoder2 = Decoder(3, 256+64)
        self.decoder3 = Decoder(4, 512+64)
        self.decoder4 = Decoder(5, 512+64)
        self.decoder5 = Decoder(6, 512+64)

        self.initialize()

    def forward(self, x, d):
        x1, x2, x3, x4, x5 = self.bkbone(x)
        d = F.interpolate(d, x1.size()[2:], mode='bilinear', align_corners=True)
        d2, d3, d4, d5     = self.depth(d)
        x_size = x.size()[2:]

        y5     = self.decoder5(torch.cat((x5, d5), 1), None)
        ...
        y4     = self.decoder4(torch.cat((x4, d4), 1), y5_4)
        ...
        y3     = self.decoder3(torch.cat((x3, d3), 1), y4_3)
        ...
        y2     = self.decoder2(torch.cat((x2, d2), 1), y3_2)
...
#            self.load_state_dict(torch.load('vgg16_bn-6c64b313.pth'))
        else:
            weight_init(self)
